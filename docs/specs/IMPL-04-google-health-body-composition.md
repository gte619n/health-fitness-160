...
| Queryable data types | Only `weight` and `body-fat` are valid `/dataTypes/{name}/dataPoints` endpoints in v4. `lean-mass` returns `INVALID_PARENT_DATA_TYPE_COLLECTION`; `bmi` is not exposed either. Both are computed values, not stored ones. The `BodyCompositionMetric` enum keeps `LEAN_MASS_KG` and `BMI` as domain concepts (so we can compute them from weight + height + body-fat ourselves later) but the integration layer only queries the two real endpoints. |
| Webhook-subscribable data types | Subset of the queryable types: `weight` and `body-fat`. Same as the queryable set in this case. |
