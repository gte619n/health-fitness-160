...
class DashboardViewModelTest {
    ...
    fun `each card transitions to Loaded independently`() = runTest(dispatcher) {
        val bodyComp = ResolvableBodyComp().apply { resolveWith(null) }
    ...
    private fun pendingBodyComp() = object : BodyCompositionRepository {
        private val pending = CompletableDeferred<WeightSummary?>()
        override suspend fun loadRecent(): WeightSummary? = pending.await()
    }
    ...
    private class ResolvableBodyComp : BodyCompositionRepository {
        private var result: Result<WeightSummary?>? = null
        fun resolveWith(value: WeightSummary?) { result = Result.success(value) }
