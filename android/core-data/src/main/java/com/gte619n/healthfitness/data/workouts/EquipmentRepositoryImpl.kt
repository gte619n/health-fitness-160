...
@Singleton
internal class EquipmentRepositoryImpl @Inject constructor(
    ...
    override suspend fun searchCatalog(
        ...
        api.search(search = search, category = category, sub = subcategory)
            .map { it.toDomain() }
    }
    ...
    override suspend fun deleteSubmission(equipmentId: String): Result<Unit> = runCatching {
        val response = api.delete(equipmentId)
        if (!response.isSuccessful) {
            throw IOException("Delete submission failed: HTTP ${response.code()}")
        }
    }
}
